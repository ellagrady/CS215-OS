...
def createDiagram():
    ...
    bars = ax.barh(1, completionTimes[0], height=0.1, label=processIDs[0], align='center', edgecolor='black',
                   linewidth=1, color='#465B6B')
    ...
    for id, val in enumerate(processIDs[:-1]):
        ...
        # add bars
        bars = ax.barh(y=1, width=completionTimes[id] - prevY, height=0.1, left=prevY, label=processIDs[id],
                       align='center', edgecolor='black', linewidth=1, color='#465B6B')
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("FCFS/RR"):
        ...
        outputBtn = gr.Textbox(label="Output", interactive=False)
        calculate.click(calculateFn, inputs=[algChoices, numProcesses, arrivalTimes, burstTimes, contextSwitch_TQ],
                        outputs=[outputBtn])
